...
select sum(Luong) as 'Tổng Lương Nhân Viên' from LUONGNV 
-- tổng lương tháng
Select Month(NgayPhatLuong) as 'Lương Tháng', Sum(Luong) as 'Tổng Lương'
From LUONGNV
Group by Month(NgayPhatLuong)
--Tổng lương nhân viên theo năm
Select Year(NgayPhatLuong) as 'Lương Năm', Sum(Luong) as 'Tổng Lương'
From LUONGNV
Group by Year(NgayPhatLuong)
select*from CTBANGLUONG
select *from LUONGNV

-- tổng nhập hàng
select sum(TienNhap) as 'Tổng tiền nhập hàng' from PHIEUNHAP 
-- tổng nhập hàng tháng
Select Month(NgayNhap) as 'Nhập Tháng', Sum(TienNhap) as 'Tổng tiền nhập'
From PHIEUNHAP
Group by Month(NgayNhap)
--Tổng nhập năm
Select Year(NgayNhap) as 'Nhập Năm', Sum(TienNhap) as 'Tổng tiền nhập'
From PHIEUNHAP
...
-- Doanh thu bán hàng
select sum(ThanhToan) as 'Doanh Thu' from HOADON 
-- tổng bán ngày
Select DAY(NgayTao) as 'Ngày bán', Sum(ThanhToan) as 'Tổng tiền bán trong ngày'
From HOADON
Group by DAY(NgayTao)
-- tổng bán tháng
Select Month(NgayTao) as 'Tháng', Sum(ThanhToan) as 'Tổng tiền bán trong tháng'
From HOADON
Group by Month(NgayTao)
--Tổng nhập năm
Select Year(NgayTao) as 'Năm', Sum(ThanhToan) as 'Tổng tiền bán trong năm'
From HOADON
...
select sum(Luong) as 'Tổng Lương Nhân Viên' from LUONGNV where Day(NgayPhatLuong) = '03'

select distinct MaQL,NgayNhap,NGUYENLIEU.MaNL,TenNL,CTPHIEUNHAP.SoLuong,DonGia,ThanhTien from PHIEUNHAP,CTPHIEUNHAP,NGUYENLIEU where PHIEUNHAP.MaPN = CTPHIEUNHAP.MaPN and CTPHIEUNHAP.MaNL = NGUYENLIEU.MaNL  
and NgayNhap between '04/09/2019' and '05/20/2019'
select*from PHIEUNHAP
